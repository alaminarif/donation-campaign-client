const CreateBlog = ({ userEmail }) => {
  ...
  return (
    <div id="contact" className="text-white h-auto w-full m-auto bg-[#222222] py-20">
      <h2 className="text-4xl mb-5 text-center">Create a blog</h2>
      <p className="text-xl text-center">Dive into your Journey of Expression and Innovation.</p>
      {!userEmail && (
        <p className="text-md text-center mt-3 text-red-500">
          You must sign in to create a post. Please{" "}
          <span className="text-blue-500 cursor-pointer" onClick={() => document.getElementById("entry_modal").showModal()}>
            sign in
          </span>{" "}
          before you start.
        </p>
      ...
      <div className="mx-auto w-11/12 lg:flex sm:block items-center">
        <form onSubmit={handleSubmit} className="xl:w-2/4 lg:w-2/4 md:w-2/4 sm:w-11/12  flex flex-col mx-auto sm:mt-10">
          ...
          {loading ? (
            <p className="text-center text-white mt-3">please wait! Uploading image takes longer than usual. Thank you for your patience.</p>
          ) : (
